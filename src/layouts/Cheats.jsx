...
const AppCard = memo(({ app, onClick, fallbackMap, onImgError, itemTheme, itemStyles }) => {
  ...
  return (
    ...
    >
      ...
      <p className="text-m font-semibold mb-3 flex-grow line-clamp-2">{app.cheatName.split('').join('\u200B')}</p>
...
const CategoryRow = memo(({ category, cheats, onClick, onViewMore, fallback, onImgError, theme, styles }) => {
...
const Cheats = memo(() => {
  ...
  useEffect(() => {
    let a = true;
    import('../data/cheats.json').then((m) => a && setData(m.default?.Cheats || {}));
    return () => {
      a = false;
    };
  }, []);
  ...
  const [dlCheats, setDlCheats] = useState([]);
  ...
  const filtered = useMemo(() => {
    ...
    if (showDl) {
      const dlNames = new Set(dlCheats.map(g => g.name));
      toFilter = all.filter(cheat => {
        const firstUrl = Array.isArray(cheat.url) ? cheat.url[0] : cheat.url;
    ...
    if (q) {
      ...
      toFilter = toFilter.filter((cheat) => {
        const cheatName = cheat.appName.toLowerCase();
        return cheatName.includes(fq);
      });
    }
    
    const total = Math.ceil(toFilter.length / perPage);
    const paged = toFilter.slice((page - 1) * perPage, page * perPage);
    return { filteredCheats: toFilter, paged, totalPages: total };
  }, [all, data, category, showDl, dlCheats, q, page, perPage]);
  ...
  const placeholder = useMemo(() => `Search ${all.length} cheats`, [all.length]);
    ...
    <div className={`${styles.appContainer} w-full mx-auto`}>
      ...
      {showDl && (
        <div className="text-center text-xs opacity-60 pb-2">
          Local cheats not played for 3+ days are automatically removed
        </div>
      )}
      ...
      {!category && !showDl && dlCount > 0 && (
        <div className="w-full flex justify-center pb-1">
          ...
          >
            View Downloaded Cheats ({dlCount})
          </button>
        </div>
      )}
      ...
      {q || category || showDl ? (
        <>
          ...
          {filtered.filteredCheats.length > perPage && (
        ...
        <div className="space-y-2">
          {Object.entries(data).map(([cat, cheats]) => (
            <CategoryRow
              key={cat}
              category={cat}
              cheats={cheats}
